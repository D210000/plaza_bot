...
const claimRequest = async (address, proxyUrl) => {
  const axiosInstance = createAxiosInstance(proxyUrl);
  try {
    const response = await axiosInstance.post(`/referrals/claim`, { address, code: "Q3SEPYnNMH7K" });
    return response.data;
  } catch (error) {
    return null;
  }
...
const claimNftReward = async ({ points, nftType, requiredPoints, wallet, proxy, claimedState }) => {
